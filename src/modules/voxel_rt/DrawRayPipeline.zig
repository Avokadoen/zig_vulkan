...
pub fn init(allocator: Allocator, ctx: Context, ray_buffer: *const GpuBufferMemory, target_image_info: ImageInfo) !DrawRayPipeline {
...

    {
        ...
        const ray_buffer_buffer_info = vk.DescriptorBufferInfo{
            ...
            .range = @floatToInt(u64, target_image_info.width) * @floatToInt(u64, target_image_info.height) * @sizeOf(Ray),
        };
...
pub inline fn dispatch(self: *DrawRayPipeline, ctx: Context, ray_process_semaphore: *vk.Semaphore) !*vk.Semaphore {
...

    {
        ...
        const compute_submit_info = vk.SubmitInfo{
            ...
            .p_wait_semaphores = @ptrCast([*]const vk.Semaphore, ray_process_semaphore),
...
pub fn recordCommandBuffer(self: DrawRayPipeline, ctx: Context) !void {
    const draw_zone = tracy.ZoneN(@src(), "draw ray compute record");
    defer draw_zone.End();
